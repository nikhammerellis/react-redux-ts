...
//action creator for loading user events (redux/thunk)
...
//action creator for creating user events (redux/thunk)
...
//get userEvents state from redux state 
const selectUserEventsState = (rootState: RootState) => rootState.userEvents;
//get list of user events via the above state selector 
export const selectUserEventsArray = (rootState: RootState) => {
    const state = selectUserEventsState(rootState);
    return state.allIds.map(id => state.byIds[id]);
};

//Reducer for User Events 
const userEventsReducer = (
    ...
    switch (action.type) {
        ...
        case UPDATE_SUCCESS:
            //renaming during restructuring (since event is used in the switch case already)
